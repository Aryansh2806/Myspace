...
for (const [theme, marker] of [
  ...
  test(`${theme} theme meets contrast minimums`, () => {
    ...
    for (const [fg, bg, min] of PAIRS) {
      ...
      // A translucent background (rgba token) is measured as it actually
      // renders: composited over the surface behind it.
      // Translucent tokens are measured as they actually render: composited
      // over what sits behind them.
      const solidBg = typeof t[bg] === "object" ? composite(t[bg], t.surface) : t[bg];
      const solidFg = typeof t[fg] === "object" ? composite(t[fg], solidBg) : t[fg];
...
for (const [theme, marker] of [
  ...
  test(`${theme}: text reads through glass over every aurora colour`, () => {
    ...
    for (const blob of ["aurora-a", "aurora-b", "aurora-c"]) {
      ...
      for (const [fg, min] of [["text", 4.5], ["muted", 4.5], ["accent", 3], ["danger", 3], ["ember", 3]]) {
