...
import { Spring, project, rubberband, velocityFrom, MAX_FLICK } from "./spring.mjs";
...
test("velocity is clamped to a plausible flick", () => {
  // Two samples in the same millisecond: dt floors at 1ms, so an unclamped
  // reading would be 130,000 px/s and would fling the element off screen.
  const v = velocityFrom([{ p: 300, t: 1000 }, { p: 170, t: 1000 }], 1000);
  assert.equal(v, -MAX_FLICK);
  assert.ok(Math.abs(v) <= MAX_FLICK);
});

test("clamping leaves ordinary flicks untouched", () => {
  // 90px in 100ms = 900 px/s, well inside the limit.
  const v = velocityFrom([{ p: 0, t: 900 }, { p: 90, t: 1000 }], 1000);
  assert.ok(Math.abs(v - 900) < 1, `got ${v}`);
});

test("a clamped flick still cannot travel absurdly far", () => {
  // Worst case projection stays within a phone-sized distance.
  assert.ok(project(MAX_FLICK) < 3200, `${project(MAX_FLICK)}`);
});
