 ...
 *
 * Clamped to a physically plausible flick. Two samples that land in the same
 * millisecond — coalesced pointer events, a throttled frame — divide by a
 * near-zero dt and produce a five-figure velocity, which then throws the
 * element clean off screen. A real thumb tops out well under this.
 */
export const MAX_FLICK = 6000;
...
export function velocityFrom(history, now, windowMs = 110) {
  ...
  const v = ((lastP.p - first.p) / dt) * 1000;
